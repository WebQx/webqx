...
Object.defineProperty(navigator, 'mediaDevices', {
  value: {
    getUserMedia: (jest.fn(() => Promise.resolve(mockMediaStream as unknown as MediaStream)) as unknown) as jest.Mock
  },
  writable: true
});
...
describe('WhisperStreamingService', () => {
  ...
  ((navigator.mediaDevices.getUserMedia as unknown) as jest.Mock).mockImplementationOnce(() => Promise.reject(error));
...
describe('Streaming Service Integration', () => {
  ...
  test('should handle API errors during processing', async () => {
    const mockWhisperService = {
      transcribeAudio: jest.fn().mockImplementation(() => Promise.reject(new Error('API Error')))
    };
